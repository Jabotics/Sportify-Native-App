...
const TabButtons = () => {
    ...
    return (
        <View className="flex flex-row mt-4 mx-4">
            <TouchableOpacity
                className={`p-1 px-2 justify-center items-center ${activeTab === 'bfu' ? 'bg-primary' : 'bg-gray-200'
                    }`}
                onPress={() => setActiveTab('bfu')}
            >
                <Text className={`${activeTab === 'bfu' ? 'text-white' : 'text-primary'} text-xs`}>
                    Best for you
                </Text>
            </TouchableOpacity>
            <TouchableOpacity
                className={`p-1 px-2 justify-center items-center ${activeTab === 'pop' ? 'bg-primary' : 'bg-gray-200'
                    }`}
                onPress={() => setActiveTab('pop')}
            >
                <Text className={`${activeTab === 'pop' ? 'text-white' : 'text-primary'} text-xs`}>
                    Popular
                </Text>
            </TouchableOpacity>
            <TouchableOpacity
                className={`p-1 px-2 justify-center items-center ${activeTab === 'near' ? 'bg-primary' : 'bg-gray-200'
                    }`}
                onPress={() => setActiveTab('near')}
            >
                <Text className={`${activeTab === 'near' ? 'text-white' : 'text-primary'} text-xs`}>
                    Nearest
                </Text>
            </TouchableOpacity>
        </View>
